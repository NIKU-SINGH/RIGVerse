...
import { Bookmark, Eye, HandCoins, Heart, Repeat, Settings } from 'lucide-react';
...
const InstagramPost: React.FC = () => {
        ...
        return (
                <div className="p-4">
                        <div className="  border-t-[1px] border-gray-700 text-gray-200 max-w-md">
                                <div className="flex items-center px-4 py-3">
                                        ...
                                        <div className="ml-3">
                                                ...
                                                <span className="text-gray-400 text-xs block">Asheville, North Carolina</span>
                                        </div>
                                </div>
                                <Image
                                        src="/games/codPoster.jpg"
                                        alt="Post Image"
                                                        ...
                                                        {comments.map((comment, index) => (
                                                                <div key={index} className="flex items-center space-x-2">
                                                                        <div className='border-2 border-pink-600 p-1 rounded-full'>
                                                                                <Avatar className="h-6 w-6">
                                                                                        {/* Replace the AvatarImage and AvatarFallback components with your actual components */}
                                                                                        <AvatarImage src={`https://github.com/${comment.username}.png`} />
                                                                                        <AvatarFallback>{comment.username[0].toUpperCase()}</AvatarFallback>
                                                                                </Avatar>
                                                                        </div>
                                                                        <div>
                                                                                <span className="text-sm font-semibold">{comment.username}</span>
                                                                                <span className="text-gray-400 text-xs block">{comment.text}</span>
                                                                        </div>
                                                                </div>
                                                        ))}
